...
const TimeEntryRow: React.FC<TimeEntryRowProps> = ({ entry, isEditing, onSave, onCancel, onEdit, onDelete, jobMap }) => {
...
// Helper functions for managing submission state in localStorage
const SUBMITTED_WEEKS_KEY_PREFIX = 'rl_submitted_weeks_';
const getSubmittedWeeksKey = (userId: string) => `${SUBMITTED_WEEKS_KEY_PREFIX}${userId}`;

const getSubmittedWeeks = (userId: string): string[] => {
  try {
    const stored = localStorage.getItem(getSubmittedWeeksKey(userId));
    return stored ? JSON.parse(stored) : [];
  } catch (e) {
    console.error("Failed to parse submitted weeks from localStorage", e);
    return [];
  }
};

const addSubmittedWeek = (userId: string, weekIdentifier: string) => {
  const current = getSubmittedWeeks(userId);
  if (!current.includes(weekIdentifier)) {
    const updated = [...current, weekIdentifier];
    localStorage.setItem(getSubmittedWeeksKey(userId), JSON.stringify(updated));
  }
};
...
export const TimesheetsPage: React.FC<TimesheetsPageProps> = ({ user }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const submittedWeeks = getSubmittedWeeks(user.id);
  ...
  const handleSubmitAndExportWeek = async (weekData: WeeklyTimeEntries) => {
    ...
    // Archive the week
    addSubmittedWeek(user.id, weekData.weekIdentifier);
